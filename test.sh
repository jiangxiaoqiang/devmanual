...
#
#TOKEN=`echo -n ${APP_ID}${APP_KEY}${CURRENT_TIME}${ECHOSTR}|md5sum|awk '{print $1}'`
...
TOKEN=`echo -n ${SORTED_TOKEN}|md5sum|awk '{print $1}'`
# 请求服务端
COMMAND=`curl -H "APPID:$APP_ID" -H "TIMESTAMP:$CURRENT_TIME" -H "ECHOSTR:$CONST_STR" -H "TOKEN:SORTED_TOKEN" \
