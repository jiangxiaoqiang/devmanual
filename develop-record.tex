...
sync子句指定全文索引的同步类型,sync transaction系统在每次事物提交后,增量填充索引,不需要手工填充索引\footnote{<<DM7\_SQL>>第3.9.1章节},手工填充填充索引语句如下：
...
如果有主备机器,可以考虑在备机上执行耗时的SQL统计语句,在备机上建立索引.这样既可以执行耗时的统计查询,同时也不影响Insert操作\cite{highperformancesql}.
...
\chapter{HTTP(HyperText Transfer Protocol)}
