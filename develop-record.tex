...
saferm.sh\footnote{\url{https://github.com/lagerspetz/linux-stuff/tree/master/scripts}} 脚本可以帮助我们将文件移动到“垃圾箱”而不是永久删除它们。如果脚本检测到 GNOME 、KDE、Unity 或 LXDE 桌面环境（DE），则它将文件或文件夹安全地移动到默认垃圾箱\$HOME/.local/share/Trash/files，否则会在您的主目录中创建垃圾箱文件夹\$HOME/Trash。
...
在Apache Maven Compiler插件中，java compiler为默认编译器，plexus(Plexus Compiler is a Plexus component to use different compilers through a uniform API) java编译器的版本太低(1.8及以下)时，会出现此问题。
...
/usr/local/pulse/PulseClient.sh -h 114.114.114.114 -u example -Url https://114.114.114.114 -r realm
/usr/local/pulse/PulseClient.sh -S
\end{lstlisting}

\subsection{离线安装Jar包}

下载离线jar包地址\footnote{\url{http://search.maven.org}}，项目在使用Maven Compiler编译的时候，Plexus包的版本是1.8，1.8版本的和Maven3.x版本结合时存在兼容性问题。Maven库连接不上，使用命令
...
项目运行时，会将依赖的项目编译后的jar包拷贝到Tomcat对应项目的lib目录下，但是其中一个依赖项目的jar包(online)始终不自动拷贝，每次都需要手工拷贝。但是依赖的拷贝配置没有问题，如图所示。
...
# 发布本地的WEB-INF文件夹到服务器
# 显示拷贝进度
rsync -avzPh WEB-INF root@192.168.1.11:/usr/local/xycq/daily-pub/diff-pub
\end{lstlisting}
