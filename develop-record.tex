...
=======
<<<<<<< HEAD
>>>>>>> 4239d2ab132cee4c68928c4d02b73575af6a82c0
...
\subsection{Spring事务(Spring Transaction)}

默认情况下，如果在事务中抛出了未检查异常（继承自 RuntimeException 的异常）或者 Error，则 Spring 将回滚事务；除此之外，Spring 不会回滚事务。rollbackFor是一组异常类的实例，遇到时必须进行回滚。
...
Peek属于中间操作(intermediate operation)，Stream中使用Peek始终不运行，非常纳闷，原来是Stream中间操作需要执行结果才会真的运行，需要流水线上触发一个终端操作(terminal operation)，否则中间操作不会执行任何处理\cite{java8inaction}。Peek操作类似于forEach操作，该方法可用于调试。Java 8流中所有的操作都是蓄而不发的，只有执行foreach，collect等终结操作时，流的操作才会执行。
...
在接收参数的的Controller中增加@InitBinder转化方法：
...
RestApiResponse为统一定义的返回实体，所有的接口都会返回标准的RestApiResponse对象，对象通过泛型定义了不同的返回内容。在客户端调用接口解析出来中文乱码,接口返回是UTF-8编码,解析时要显示指定编码.
...
脚本的目的是要使用不同的时间，随机生成的字符串，保证每次请求的Token都需要不一样，相当于一次一密。在使用shasum命令时，有时可能会没有安装，在CentOS下输入如下命令安装即可：

\begin{lstlisting}[language=Bash]
yum install perl-Digest-SHA -y
\end{lstlisting}

一次性密码本是一种理论上不可能破解的密码本\cite{secrettech}.使用如下脚本生成随机token：
...
\subsection{Thread starvation or clock leap detected}
...
\paragraph{Unable to set localhost. This prevents creation of a GUID}

项目启动时报错,Unable to set localhost. This prevents creation of a GUID. Cause was: cldap: cldap: Temporary failure in name resolution java.net.UnknownHostException: cldap: cldap: Temporary failure in name resolution.是由于服务器修改了主机名称的缘故,在hosts文件中添加新主机名即可:
...
mybatis-spring-boot-starter 1.3.x版本对应Spring Boot 1.5.x版本,Sping、Mybati与Spring MyBatis具体的版本对应关系可以参考这里\footnote{\url{http://www.mybatis.org/spring/}}，MyBatis他与Spring Boot的对应关系暂时未找到。
...
参数传入List即可，注意列表的数量不宜过大，一般数据库对传入的参数总个数有限制，也就是同时一批写入的数据量不能太大。
...
需要打印格式化的SQL,再properties文件中添加如下配置:
...
需要显示SQL绑定的参数，再log4j.properties中做如下配置：
...
\subsection{集合操作(Collection Operation)}
...
\section{sbt(Scala Build Tool)}

\subsection{Error during sbt execution: No Scala version specified or detected}
...
Its the repository cache thats causing the issue. There are possibilities where you have broken / no ivys (when only the jar is downloaded through maven repo) for the sbt jars in your cache.移除~/.sbt/boot目录。再次执行sbt命令即可，但是需要注意的是jcenter和maven-central下载包都会非常慢，将repositories路径配置为阿里云路径，可大大提高下载速度：
...
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.4]{reduxdataflow.png}
	\caption{Redux数据流}
...
如果字段有数字，直接转换为小写匹配，如果没有数字，将字段名字转换为骆驼命名去匹配：
...
再使用npm的时候，部署的时候遇到开发版本和实际部署版本的细微差异导致的问题。而yarn为了避免包版本的错误匹配，一个确定的安装版本被固定在一个锁文件中。每次模块被添加时，Yarn 就会创建（或更新）yarn.lock 文件，这样你就可以保证其它机子也安装相同版本的包，同时包含了 package.json 中定义的一系列允许的版本。级
...
有时在浏览器可能需要跟踪Javascript代码，在配置文件webpack.config.js中添加如下语句：

\begin{lstlisting}[language=JavaScript]
devtool: 'cheap-module-source-map',
\end{lstlisting}

可以在服务端看到文件的目录树结构，如图\ref{fig:serverfiletree}所示．

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.5]{serverfiletree.png}
	\caption{网站文件树}
	\label{fig:serverfiletree}
\end{figure}
...
\begin{lstlisting}[language=Bash]
#查看antd的版本(需要到开发程序对应的目录下)
npm ls antd
\end{lstlisting}

有时在使用npm安装包的过程中,需要等待很长时间,此时就非常需要查看安装进度,可以做如下设置:
...
f(fix)参数表示尝试修正系统依赖损坏处。在Linux常用的重要文件如下表\ref{table:linuxconfigfile}所示。配置文件存放的目录名etc，本意为“etcetera”(附加物)的意思。
...
Linux系统中单个进程的最大线程数有其最大的限制PTHREAD\_THREADS\_MAX,这个限制可以在 /usr/include/bits/local\_lim.h 中查看:
...
\paragraph{创建路径(Create Directory)}创建目录及目录树可以批量进行，一个命令，即可创建整个目录树。
...
\paragraph{读取配置文件(Read Config File)}

在编写自动化部署脚本时，需要通过shell脚本读取配置文件version.properties中的版本号，直接使用source命令即可：

\begin{lstlisting}[language=Bash]
# properties文件中直接写VERSION\_CODE=1.0.2即可
...
\subsection{常用Bash脚本(Bash Snippnet)}
...
编写Bash脚本时注意在Windows下编写的脚本默认是dos格式的，到Linux下执行时，需要手工调整为Unix格式。



\paragraph{结束指定进程}

结束指定名称的进程,在系统部署前,需要结束当前运行的进程:

\begin{lstlisting}[language=Bash]
#!/bin/sh

readonly PROCESS_NAME="credit-system-web-api"
count=`ps -ef | grep ${PROCESS_NAME} | grep -v "grep" | wc -l`
# 进程是否存在
if [ $count -eq 1 ]; then
	# 根据进程名称找到进程ID
	PROCESS_ID=$(ps x | grep ${PROCESS_NAME} | grep -v grep | awk '{print $1}')
	if [ ${PROCESS_ID} -gt 100 ]; then
		kill ${PROCESS_ID}
	fi
fi
\end{lstlisting}
...
\begin{table}
	...
	\begin{center}
		\begin{tabular}{cp{12cm}c}
			...
			\hline
			Alt + F10 & Alt+F10在Xface下会最大化当前窗口　\\						
			\hline
		\end{tabular}
	\end{center}
\end{table}

有时需要执行自定义脚本,比如发布项目时需要执行发布脚本,每次切换目录都非常麻烦,可以将脚本命令设置别名,直接可以在任意目录下通过运行命令的方式即可执行发布项目的脚本,在bashrc文件中添加如下别名定义:

\begin{lstlisting}[language=Bash]
alias pubtest='bash /home/hldev/hldata/backend/credit-system-v1.3/script/deploy/onekey-deploy.sh'
\end{lstlisting}

设置完成后在任意路径下直接执行pubtest命令即可发布项目.
...
\begin{lstlisting}[language=Bash]
ps -ef|grep -w 'dolphin-web'|grep -v grep|cut -c 9-15|xargs kill 9
\end{lstlisting}

\paragraph{rm(remove)}

saferm.sh\footnote{\url{https://github.com/lagerspetz/linux-stuff/tree/master/scripts}} 脚本可以帮助我们将文件移动到“垃圾箱”而不是永久删除它们。如果脚本检测到 GNOME 、KDE、Unity 或 LXDE 桌面环境（DE），则它将文件或文件夹安全地移动到默认垃圾箱\$HOME/.local/share/Trash/files，否则会在您的主目录中创建垃圾箱文件夹\$HOME/Trash。使用如下命令将rm命令添加别名：
...
nslookup命令用于查询DNS的记录，查看域名解析是否正常，在网络故障的时候用来诊断网络问题。nslookup is a network administration command-line tool available for many computer operating systems for querying the Domain Name System (DNS) to obtain domain name or IP address mapping or for any other specific DNS record.

\begin{lstlisting}[language=Bash]
nslookup {domain}
\end{lstlisting}

查询结果如图\ref{fig:nslookup}所示。

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.6]{nslookup.png}
	\caption{nslookup查看域名解析情况}
	\label{fig:nslookup}
\end{figure}
...
\begin{lstlisting}[language=Bash]
sudo dd bs=4M if=/home/hldev/software/cn_windows_10.iso of=/dev/sdb4
\end{lstlisting}
...
\begin{lstlisting}[language=Bash]
# 让管道自动输入一大堆得yes，就可以完成了强行复制
yes | cp -fr src dest
\end{lstlisting}


\paragraph{nohup(No hang up)}
...
# 列出大于200MB的文件
# 排除home目录
find / \( -path "/home" \) -prune -o -type f -size +200M -print
\end{lstlisting}
...
Phil Katz取名格式为zip格式，是表示此种压缩算法速度快的特点，单词zip本身就有表示快速移动的含义。解压tar.xz压缩文件，先xz -d xxx.tar.xz将xxx.tar.xz解压成xxx.tar.然后，再用tar xvf xxx.tar来解包。xz是一个使用LZMA压缩算法的无损数据压缩文件格式。和gzip与bzip2一样，同样支持多文件压缩，但是约定不能将多于一个的目标文件压缩进同一个档案文件。相反，xz通常作为一种归档文件自身的压缩格式，例如使用tar或cpioUnix程序创建的归档。xz格式的文件解压时较短。
...
但是有时创建压缩档时，需要排除制定目录和指定类型的文件:

\begin{lstlisting}[language=Bash]
tar -zcvf tomcat.tar.gz --exclude=tomcat/logs --exclude=tomcat/libs tomcat
tar -cvf test.tgz test/ --exclude *.jpg
# 压缩nginx文件夹到nginx.tar.gz
# 排除日志文件
tar -zxvf nginx.tar.gz --exclude=*.log nginx
\end{lstlisting}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.7]{grepfilterreturnvalue.png}
...
如果需要同时排除多个文件夹，命令后跟多个--exclude-dir命令即可。打印匹配行的前后几行：

\begin{lstlisting}[language=Bash]
# 打印匹配行的前后5行
tail -f cc-web.2018-07-05.0.log |grep -C 5 "ERROR"
# 打印匹配行的后5行
tail -f cc-web.2018-07-05.0.log |grep -A 5 "ERROR"
\end{lstlisting}
...
在定时任务配置文件/etc/cron.d/sysstat中定义数据搜集的频率:
...
设置采集数据保留的时长在Ubuntu下是/etc/sysstat/sysstat文件,在CentOS下是/etc/sysconfig/sysstat文件:
...
pstree命令以树状图显示进程间的关系（display a tree of processes）。查看当前系统总的线程信息:
...
如果目标系统在3秒的超时间隔内没有响应，所有的查询都会发生超时，结果会采用星号（*）显示，由此可以断定是目标系统的问题。
...
\paragraph{du(Disk Usage)}

Linux du命令是对文件和目录磁盘使用的空间的查看.查看software文件夹的大小：
...
有时需要排除某类文件，比如需要排除日志文件：

\begin{lstlisting}[language=Bash]
# 查看nginx文件夹大小
# 排除日志文件
du -sh --exclude="*.log" nginx
\end{lstlisting}
...
\begin{lstlisting}[language=Bash]
# 挂载U盘到系统
sudo mount -t vfat /dev/sdb1 /media/hldev/udisk/
# 卸载U盘
sudo umount /media/hldev/KINGSTON
\end{lstlisting}

\paragraph{时间(Time)}

有时,会遇到Linux时间错误的问题,此时就需要手动设置时间或者自动从时间服务器上同步时间.由于服务器是物理内网,所以手动设置时间. Linux系统时钟有两个，一个是硬件时钟，即BIOS时间，就是我们进行CMOS设置时看到的时间，另一个是系统时钟，是Linux系统Kernel时间。手动设置时间命令如下:

\begin{lstlisting}[language=Bash]
# 设置软件时间
# 设置软件时间为2018年3月26日11点58分46秒
# 月 日 时 分 年. 秒
date 032611582018.46
# 同步到硬件
# Set the Hardware Clock to the current System Time
# System time sync to hardware
hwclock -w
hwclock --systohc
\end{lstlisting}
...
#unix name的意思
uname －a 
#LSB（Linux 标准库Linux Standard Base）
#能够打印发行版的具体信息
#包括发行版名称、版本号、代号等
lsb_release -a
#这个文件记录了Linux内核的版本
...
Linux里的lsb\_release命令用来查看当前系统的发行版信息（prints certain LSB (Linux Standard Base) and Distribution information）。有了这个命令就可以清楚的知道到底是RedHat的、还是别的发行版，还有具体的版本号.
...
mkdir -p dolphin
# 挂载U盘
mount /dev/sdb1 dolphin
\end{lstlisting}


\subsection{Linux进程}

查看程序的目录，需要找到进程的PID,进入到PID对应的目录，使用ll命令,exe文件对应的链接即为包含了正在进程中运行的程序链接。
...
占用资源较GNOME ，KDE较少。适合老机器，轻量级桌面。与windows界面环境类似。许多不习惯GNOME 3 ，Unity新桌面的同学，很多选择了XFCE 4.8，包括Linus大神同学。安装Xfce后,需要使用startx启动进入Xfce,执行startx时,实际运行指令:
...
调整DNS,在Ubuntu 16.04 LTS下,新建文件/run/resolvconf/resolv.conf(因为etc目录下的resolv.conf文件是链接到此文件的),在文件中添加dns配置:
...
添加配置(不需要重启网络)后,dns解析就生效了,直接ping域名也可以.配置网络常用命令:
...
Java进程运行一段时间后自动结束，也没有错误日志输出。也没有提示内存溢出什么的。查看Linux系统日志可以找到原因：

\begin{lstlisting}[language=Bash]
grep -R "Out of" /var/log
\end{lstlisting}

Linux内存不足时自动结束进程如图\ref{fig:linuxkillprocess}所示。

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.5]{linuxkillprocess.jpg}
	\caption{内存不足时自动结束进程}
	\label{fig:linuxkillprocess}
\end{figure}

\subsection{too many open files}

出现这句提示的原因是程序打开的文件/socket连接数量超过系统设定值。查看每个用户最大允许打开文件数量:
...
在Linux下卸载Intellij Idea找到安装目录后直接删除目录即可。重装Intellij Idea后调试时进入断点即可看到相应的变量，如图\ref{fig:probescalavariable}所示。
...
Intellij　Idea图标含义可以在官网\footnote{\url{http://www.jetbrains.com/help/idea/symbols.html}}查看，有时出现了橘黄色小图标，中间带一个J符号，选择项目启动类时也无法选择，因为图标含义表示此文件不是Source文件，自然无法选择．原因是由于项目未把该路径指定为源码路径。解决方法：在Project Structure中（快捷键Ctrl+Alt+Shift+S）选Modules将你带红圈的文件添加上，将你带红圈的文件夹设为Sources。一般是src文件夹，点Sources文件夹，然后点击OK。
...
在使用Intellij Idea的过程中，有时总是不出现智能提示，不出现智能提示大致分为3种情况。其一是勾选了省电模式，在省电模式下没有智能提示。第二是缓存问题，解决办法删除缓存 file -> invalidate caches，根据提示重启Intellij Idea。 情况三 配色问题。解决方法是，切换到其他配色，比如内置配色darchla。自己遇到的情况是属于勾选了省电模式。
...
\begin{lstlisting}[language=Bash]
/path/to/tomcat/catalina.sh version
\end{lstlisting}

appBase:可以指定绝对目录,也可以指定相对于<CATALINA\_HOME>的相对目录.如果没有此项,默认为<CATALINA\_HOME>/webapps

\chapter{Maven}

\section{基础问题}

将项目从Eclipse中移除后又重新导入：

\begin{lstlisting}[language=Bash]
mvn clean eclipse:clean eclipse:eclipse
# 列出ClassPath引用所有的jar包
mvn dependency:list
# 拷贝出来依赖的jar包
...
\subsection{java.lang.NoSuchMethodError}

在使用Maven install命令时，提示错误：
...
在Apache Maven Compiler插件中，javac compiler为默认编译器，plexus(Plexus Compiler is a Plexus component to use different compilers through a uniform API) java编译器的版本太低(1.8及以下)时，会出现此问题。解决此问题可以将默认的javac编译器替换为eclipse编译器，且plexus的版本调整为2.x版本。

\subsection{调整编译版本}

maven3在编译的时候，默认使用的jdk1.5进行编译的。


\subsection{部署后去掉项目名}

一般在使用Maven插件打包成war包部署到Tomcat后，访问路径需要带上项目名称，但是带上项目名称后部分静态资源无法成功加载，在Tomcat的conf文件夹下的server.xml文件的Host配置节中可以添加如下配置以去掉项目名称：

\begin{lstlisting}[language=XML]
<Context docBase="cqnews-cms" path="/" reloadable="true" source="org.eclipse.jst.j2ee.server:cqnews-cms"/>
\end{lstlisting}

添加配置后访问URL中就可以不带项目名称。
...
下载离线jar包地址\footnote{\url{http://search.maven.org}}，项目在使用Maven Compiler编译的时候，Plexus包的版本是1.8，1.8版本的和Maven 3.x版本结合时存在兼容性问题。Maven库连接不上，使用命令

\begin{lstlisting}[language=Bash]
mvn install:install-file -Dfile=d:/scala-maven-plugin-3.2.2.jar -DgroupId=net.alchim31.maven -DartifactId=scala-maven-plugin -Dversion=3.2.2 -Dpackaging=jar
\end{lstlisting}

\subsection{引用本地Jar包}

Maven有于网络原因，暂时无法连接Maven私有仓库，在Maven项目中使用本地JAR包：

\begin{lstlisting}[language=XML]
<dependency>
	<groupId>com.cqliving</groupId>
	<artifactId>cqliving-redis</artifactId>
	<version>2.0.0-SNAPSHOT</version>
	<scope>system</scope>	
	<systemPath>/var/lib/jenkins/workspace/exmaple/cqliving-redis/target/cqliving-redis-2.0.0-SNAPSHOT.jar</systemPath>
</dependency>
\end{lstlisting}

scope必须填写为system，而systemPath为引用jar包的绝对路径，注意不能是路径的目录，需要是全路径，要包含文件名。

\subsection{Maven Compiler指定版本}

有于Maven Compiler默认版本存在不兼容情况，编译时需要手动指定编译器：
...
\subsection{拷贝jar包到lib目录}

项目运行时，会将依赖的项目编译后的jar包拷贝到Tomcat对应项目的lib目录下，但是其中一个依赖项目的jar包(online)始终不自动拷贝，每次都需要手工拷贝。但是依赖的拷贝配置没有问题，如图\ref{fig:assemblydeployment}所示。
...
在使用Gradle命令构建项目时，提示错误如下：Could not find or load main class GradleWrapperMain。

\begin{lstlisting}[language=Bash]
./gradlew -p credit-report-web-boot -x test build
\end{lstlisting}

The reason is there is no gradle/wrapper/gradle-wrapper.jar in repository. It should not be in .gitignore.将jar包拷贝到当前项目对应的目录下即可。

\paragraph{temporary failure in name resolution}

在使用Gradle下载文件时，提示temporary failure in name resolution。表示域名解析失败，编辑文件/etc/resolv.conf，添加Google域名解析地址。
...
\subsection{搭建家庭多媒体中心}

OpenELEC 是一个基于 Linux 的 开源 多媒体播放系统。同类的系统还有 OSMC，OSMC 的系统安装和使用方式和 OpenELEC 差别不大。目前 OpenELEC 支持 Raspberry Pi1、Pi2、Pi3 以及 Zero。大家可以很轻松的通过 OpenELEC 来搭建家庭的多媒体中心。
...
Couldn't run /usr/sbin/dumpcap in child process: Permission denied`.添加对dumpcap(Dump Capture)文件的的执行权限：
...
1的含义是文件的第一行,?的含义是反向查找,feedback的含义是要查找的关键字.

\paragraph{替换(Replace)}
...
(注：m和n 为数字，若m为 .，表示为当前行开始；若n为\$，则表示到最后一行结束) u
...
再使用wget时，远程服务器上的tar.gz格式的文件下载后变成HTML格式，导致解压失败。使用curl命令下载即可：

\begin{lstlisting}[language=Bash]
# 使用Curl下载，避免wget下载改变文件类型
curl -O http://192.168.0.14/assets/templatefile/haishu.tar.gz
\end{lstlisting}


\subsection{7zip}

7-zip以高压缩率著称，并且是一款免费开源的压缩软件。
...
rsync在备份网站时也非常有帮助，通常网站下面会有许多log文件、图片等，一般都非常大,GB级别，但是一般情况下的备份不需要备份这些文件。
在发布文件时，网速比较慢，发布文件上传需要较长时间，rsync工具很有用。启动守护进程：

\begin{lstlisting}[language=Bash]
# 如果没有/var/rsync文件夹
# 手工创建
/usr/local/bin/rsync --daemon start
\end{lstlisting}
...
\begin{lstlisting}[language=Bash]
# 发布本地的WEB-INF文件夹到服务器
rsync -avz WEB-INF root@192.168.1.11:/usr/local/daily-pub/diff-pub
\end{lstlisting}

第一次发布时，会全量拷贝文件，再次发布时，只拷贝变动过的文件，在网速较慢的环境中，此项特性可以节省大量时间。archive归档模式，a表示以递归方式传输文件，并保持所有文件属性。v表示verbose详细模式输出。z表示compress，对备份的文件在传输时进行压缩处理。如下命令可以查看文件拷贝的进度：

\begin{lstlisting}[language=Bash]
# 发布本地的WEB-INF文件夹到服务器
# 显示拷贝进度
rsync -avzPh WEB-INF resource root@192.168.1.11:/usr/local/xycq/daily-pub/diff-pub
\end{lstlisting}

P参数表示显示拷贝的进度。h参数会以友好的方式(Human Readable)显示已经拷贝文件总大小。在使用SFTP工具拷贝时，总是会因为文件太大，拷贝的时间过长而导致超时，使用rsync则不存在此问题。

\subsection{sftp}

SFTP ，即SSH 文件传输协议（ SSH File Transfer Protocol ），或者说是安全文件传输协议（ Secure File Transfer Protocol ）。 SFTP 是一个独立的SSH 封装协议包，通过安全连接以相似的方式工作。
...
vsftpd 是“very secure FTP damon”的缩写，安全性是它的一个最大的特点。vsftpd是一个UNIX类操作系统上运行的服务器的名字，它可以运行在诸如Linux，BSD，Solaris，HP-UNIX等系统上面，是一个完全免费的，开发源码的FTP服务器软件。

\begin{lstlisting}[language=Bash]
/bin/systemctl start vsftpd.service俄
\end{lstlisting}
...
Ansible主机配置,在配置文件/etc/ansible/hosts中：
...
Ansible主机简单分组配置示例如图所示:

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.4]{ansiblehostconfigexample.png}
	\caption{Ansible主机配置实例}
	\label{fig:ansiblehostconfigexample}
\end{figure}
...
\subsection{timed out waiting for input: auto-logout}

ssh登录一会儿后，自动断开，提示timed out waiting for input: auto-logout。此时需要设置TMOUT变量。在home目录下的文件.bash\_profile中，设置如下常量：

\begin{lstlisting}[language=Bash]
TMOUT=100
\end{lstlisting}

这会让用户在停止动作 100 秒后自动登出。If set to a value greater than zero, TMOUT\footnote{\url{http://man7.org/linux/man-pages/man1/bash.1.html}} is treated as the
...
# 本地端口转发实例
#在10.20.1.24上执行
#访问本地10.20.1.24的8888端口就相当于访问远程10.20.1.11的8083端口
ssh -g -L 8888:10.20.1.11:8083 10.20.1.24
\end{lstlisting}
...
可以根据客户端的 user-agents 首部字段来阻止指定的爬虫爬取我们的网站。使用如下命令查看最近访问IP，以访问频率倒序排列：

\begin{lstlisting}[language=Bash]
awk '{print $1}' /usr/local/nginx/logs/new.xycq.gov.cn.log |sort |uniq -c|sort -nr
\end{lstlisting}

对方使用的估计是阿里云的动态IP，太多了，限制IP段访问：

\begin{lstlisting}[language=Bash]
# 屏蔽整个39.106.173.* C段IP
deny 39.106.173.0/24；
# 屏蔽整个39.106.0.1到39.106.255.254  
deny 39.106.173.0/24； 
# 封整个段
# 从118.255.255.1到118.255.255.254的命令
deny 118.0.0.0/8   
\end{lstlisting}
...
使用Ngin绑定多个域名后，由于多个域名都是用的80端口，内网无法直接通过80端口访问。所以采取复制一份配置的方案，复制的配置单独指定一个独立的端口，配置项与公网的配置项保持一致。由于添加了安全策略会导致公网访问和内网访问的效果不一致，这样既可以公网访问，也可以内网访问，方便问题的排查。
...
如果仓库太大，可以在git clone中加入参数--depth=1，只拉取最近的一个revision。
...
\subsection{Git使用常见问题}

在使用不同分支部署时，由于Git默认忽略了build和out文件夹，所以build和out文件夹可能会有其他分支编译的文件，当在打包发布时，可能会由于打包进其他分支的文件或者目录导致部署时启动失败，所以部署时不同分支的文件最好单独存放在不同的文件夹中。
...
\paragraph{You are not allowed to push code to this project}

push代码时，提示如下错误：

\begin{lstlisting}[language=Bash]
remote: You are not allowed to push code to this project.
fatal: unable to access 'https://gitlab.hualongdata.com/haibo.zeng/credit-report.git/': The requested URL returned error: 403
\end{lstlisting}
...
Chromium是一个由Google主导开发的网页浏览器，以BSD许可证等多重自由版权发行并开放源代码\footnote{\url{https://download-chromium.appspot.com/}}。Chromium的开发可能早自2006年即开始。
...
Google Chrome版本安装插件时，提示错误Apps, Extensions and scripts cannot be added from this website，Google为了推行谷歌的扩展商店所做的限制导致了此问题。使用如下命令启动即可顺利安装插件：

\begin{lstlisting}[language=Bash]
google-chrome --enable-easy-off-store-extension-install
# 如果要顺畅安装插件，在Google Chrome启动参数后加上
# enable-easy-off-store-extension-install参数
...
有的时候需要截取一个下拉网页，但是需要向下滑动才能够截取全部，在Chrome 59中已经支持对整个页面进行截屏,新版的Google Chrome中可以使用Ctrl + Shift + P(Program)打开程序运行窗口，输入Capture，选择Capture full size screenshot即可,如果没有需要的屏幕尺寸,需要手动添加自定义尺寸。
...
奇怪的是，这种方式在Mac OS X下有效，但是在Ubuntu 16.04 LTS下无效。在Fedora 28下有效果。

\paragraph{换行}Latex的自动拆单词换行是依靠字典的。如果遇到\LaTeX{}不认识的单词，他就不会换行了。后面的内容就会溢出。在Url无法正确换行时,hyperref宏包提供了两个宏，\textbackslash UrlBreaks 以及\textbackslash UrlBigBreaks，用于告知 TeX，用户允许在何处截断 URL 以便换行。添加如下代码\footnote{\url{https://liam0205.me/2017/05/17/help-the-url-command-from-hyperref-to-break-at-line-wrapping-point/}},Url即可换行:
...
\subsection{文献引用(Bibliography Reference)}
...
\chapter{嵌入式(Embedded)}

\section{Redis}

\subsection{MISCONF Redis is configured to save RDB snapshots}
...
运行一段时间之后报错：MISCONF Redis is configured to save RDB snapshots。导致此问题的原因是Redis在保存数据时出现错误，默认情况下为了安全，防止无法有效意识到错误，导致造成更大的问题，默认在持久化数据出现问题时，不再接受任何写操作。而本项目中Redis保存的数据重要性不高，所以关闭此特性即可。
...
有时为了避免第一次请求慢的问题，希望缓存不经过HTTP请求也能够自动刷新，参考文章：https://zentao.hualongdata.com/zentao/bug-view-4418.html。
...
\subsection{索引常识(Commonsense of Index)}
...
\subsubsection{索引匹配原则(Index Match Principle)}
...
\paragraph{=和in可以乱序}=和in可以乱序，比如a = 1 and b = 2 and c = 3 建立(a,b,c)索引可以任意顺序，mysql的查询优化器会帮你优化成索引可以识别的形式。

\paragraph{尽量选择区分度高的列作为索引}尽量选择区分度高的列作为索引,区分度的公式是count(distinct col)/count(*)，表示字段不重复的比例，比例越大我们扫描的记录数越少，唯一键的区分度是1，而一些状态、性别字段可能在大数据面前区分度就是0，那可能有人会问，这个比例有什么经验值吗？使用场景不同，这个值也很难确定，一般需要join的字段我们都要求是0.1以上，即平均1条扫描10条记录。有的时候索引选择器会根据where子句和order by子句来计算索引的消耗，有可能不会走最佳索引，例如：
...
此时根据ROWID发现走聚集索引代价低，所以DFBM索引会失效。

\paragraph{索引列不能参与计算}索引列不能参与计算，保持列“干净”，比如from\_unixtime(create\_time) = ’2014-05-29’就不能使用到索引，原因很简单，b+树中存的都是数据表中的字段值，但进行检索时，需要把所有元素都应用函数才能比较，显然成本太大。所以语句应该写成create\_time = unix\_timestamp(’2014-05-29’);

\paragraph{尽量的扩展索引}尽量的扩展索引，不要新建索引。比如表中已经有a的索引，现在要加(a,b)的索引，那么只需要修改原来的索引即可
...
全文索引是基于要编制索引的文本中的各个标记来生成倒排序、堆积且压缩的索引结构。每个表或索引视图只允许有一个全文索引。该索引最多可包含 1024 列。 
...
\subsection{安装pgAdmin 4}

下载安装包后,运行命令:

\begin{lstlisting}[language=Bash]
#安装PostgreSQL
sudo pip2 install pgadmin4-2.1-py2.py3-none-any.whl
#切换到安装目录5a5c7c70290000e927486750
cd /usr/local/lib/python2.7/dist-packages/pgadmin4
#启动pgAdmin4
#初次启动需要设置用户名密码
#暂时设置用户名为jiangtingqiang@gmail.com
...
\begin{lstlisting}[language=Bash]
./pg_dump -h 59.214.215.6 -U haishu msgmanage > msgmanage.bak
\end{lstlisting}

Postgres数据还原命令如下：

\begin{lstlisting}[language=Bash]
./psql -h localhost -U haishu -d msgmanage < msgmanage.bak　
\end{lstlisting}
...
此种备份方式会遗失表的约束等，推荐先采用创建表语句创建数据表，再使用insert语句将数据写入表中．


\subsection{常用语句}

数据写入SQL,将ig\_app\_user\_201805121436表中的数据写入ig\_app\_user：

\begin{lstlisting}[language=SQL]
insert into ig_app_user
(
	app_id,
	passport_id,
	user_id,
...
select * from ig_app_user_201805121436;
\end{lstlisting}
...
\begin{lstlisting}[language=Bash]
./psql postgres -h 127.0.0.1 -d postgres
\end{lstlisting}
...
mysql使用unix socket或者tcp来连接数据库进行通讯，默认不加 -h选项时使用的就是localhost即Unix Socket，此时会通过/tmp/mysql.sock来通讯，但是在配置文件中默认生成的socket文件是在/var/lib/mysql/mysql.sock(不同安装可能不同，建议查看/etc/my.cnf确认),所以要想mysql使用这个文件通讯，最简单的方法就是建立软链接，一劳永逸，此为方法一
...
\begin{lstlisting}[language=SQL]
select substr(content_text,instr(content_text,'机密',1,1)-20,30) 
from "INFORMATION"
where cast(content_text as varchar(800)) like '%机密%'
limit 1000;
\end{lstlisting}
...
\begin{lstlisting}[language=Bash]
call sp_tabledef('USER_XYCQ','TD_M_CREDENTIAL');
\end{lstlisting}
...
\subsection{form-data、x-www-form-urlencoded、raw、binary的区别}

http请求中的multipart/form-data,它会将表单的数据处理为一条消息，以标签为单元，用分隔符分开。既可以上传键值对，也可以上传文件。当上传的字段是文件时，会有Content-Type来说明文件类型；content-disposition，用来说明字段的一些信息；由于有boundary隔离，所以multipart/form-data既可以上传文件，也可以上传键值对，它采用了键值对的方式，所以可以上传多个文件。就是application/x-www-from-urlencoded,会将表单内的数据转换为键值对，比如,name=java\&age = 23.raw可以上传任意格式的文本，可以上传text、json、xml、html等.binary相当于Content-Type:application/octet-stream,从字面意思得知，只可以上传二进制数据，通常用来上传文件，由于没有键值，所以，一次只能上传一个文件。
...
以上是将"玖龙纸业"四个汉字的UTF-8编码以8进制的方式表示.每个汉字用3个字节表示,347|216|226表示汉字"玖",后面的表示方式以此类推.采用URLEncode编码方式获得的是16进制的显示方式:

\begin{lstlisting}[language=Java]
String url = "玖龙纸业";
/**
*URLEncoder输出的结果(16进制):
*%E7%8E%96%E9%BE%99%E7%BA%B8%E4%B8%9A
*将%替换为\,16进制的E7转换为8进制即为\347
*即上面的表示方式
*/
...
\section{性能分析(Performance Analysis)}

\subsection{慢的SQL查询}

在项目中报表打印页面非常慢，需要10几秒才能出结果。打开Network，找到所有请求按照时间排列发现耗时最多的一个请求，针对此请求进行优化。在SQL中添加相应的索引可以解决。

\subsection{大的Javascript脚本}

在项目中，发现有的JavaScript脚本特别大，有6MB+之巨，明显是造成性能瓶颈的原因之一，如图\ref{fig:largejavasript}所示。
...
\subsection{Stalled时间很长}

页面发送的TCP请求较多,发现后发送的TCP请求Stalled的时间很长.发生Stalled的情况大致如下,一是有高优先级的请求,二是一个源下已经有6个TCP连接,那么后面的必须等待前面的请求完毕后,才能发送,这段时间就是Stalled时间,第三种情况是浏览器在分配空间\footnote{\url{https://developers.google.com/web/tools/chrome-devtools/network-performance/reference\#timing-explanation}}.
...
\section{通用规范(Specification)}

\subsection{版本管理(Version Control)}
...
\subsection{问题排查(4A登录)}

系统登录的流程是，在登录主APP时会向统一向后端发送登录请求(auth/signon)，Nginx将请求转发给统一认证APP,APP将各个系统登录信息加密后设置在Cookie中，如图所示．

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.4]{allsystemcookies.png}
	\caption{各个系统的加密Cookie}
	\label{fig:allsystemcookies}
\end{figure}

这样登录各个系统时，就能够根据每个系统的标识，获取到用户的授权信息，从而实现自动登录．当自动登录失败时，可以到各个子系统看打印日志，确定子系统从Cookie中获取到了对应的正确的用户信息，如图\ref{fig:traceloginusercookie}所示．

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.4]{traceloginusercookie.png}
	\caption{跟踪登录的用户Cookie信息}
	\label{fig:traceloginusercookie}
\end{figure}

在登录系统时，有的时候会提示跨域，要注意登录地址要通过本系统的Nginx转发，而不能直接请求4A的登录地址．

\subsection{问题归集}

配置需要注意的问题点，Ngnix后端转发解决跨域问题：

\begin{lstlisting}[language=Bash]
#测试环境系统的登录地址
#配置时需要注意要用本域的后端Nginx做转发
#不能直接配置登录地址
#否则会存在跨域问题
as.config.loginPage=http://ip:port/auth/signon
\end{lstlisting}
...
\begin{lstlisting}[language=Java]
JSONObject json = new JSONObject(responseContent);
String total = json.getString("totalElements");
\end{lstlisting}
...
\begin{lstlisting}[language=Bash]
nohup sudo /opt/dmdbms/bin/dmserver /opt/dmdbms/data/DAMENG/dm.ini -noconsole &
sudo ./DmServiceGRP1_RWW_01 start
\end{lstlisting}
...
\begin{lstlisting}[language=Bash]
sudo dd bs=4M if=/home/hldev/software/cn_windows_10_multi-edition_version_1709_updated_dec_2017_x64_dvd_100406696.iso of=/dev/sdb4
\end{lstlisting}
...
\begin{lstlisting}[language=Bash]
awk '{print $1}' /home/nginx/logs/new.xycq.gov.cn.log |sort |uniq -c|sort -nr|head -n 20
\end{lstlisting}
...
\begin{lstlisting}[language=Bash]
# 直接启动达梦
nohup /home/hldev/dmdbms/bin/dmserver /home/hldev/dmdbms/data/DAMENG/dm.ini -noconsole &
...
\end{document}
>>>>>>> a66fe9cf1b662b809c0e510360395d6d8ab18eae
>>>>>>> 4239d2ab132cee4c68928c4d02b73575af6a82c0
