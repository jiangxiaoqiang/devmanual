...
Nginx作为HTTP代理转发前端时，后端服务无法获知前端访问客户的IP地址。要获取客户端IP，需要编译http\_realip\_module。查看Nginx安装了哪些模块，使用如下命令：
...
此处显示已经编译了http\_realip\_module模块。如果没有编译http\_realip\_module:
...
xrdp支持不了13.10的GNOME了，解决办法是安装xfce界面。被远程的机器上安装必须的组件：
...
在Ubuntu上使用Remmina Remote Desktop Client即可链接远程桌面。
