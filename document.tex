...
在使用nohup时，有一个缺点就是交互性很低，例如后台启动OpenVPN时，需要输入预先设置的认证密码。但是使用nohup命令启动时，无法显示密码输入入口。此时就需要使用screen命令了。输入如下的命令：

\begin{lstlisting}[language=Bash]
screen sudo openvpn client.conf
\end{lstlisting}

接着在跳出的窗口中，输入密码。按下Ctrl + A后再按下d键，暂时断开screen会话即可。
...
为了暂时激活autojump应用，即直到你关闭当前会话或打开一个新的会话之前让autojump均有效，需要以常规用户身份运行下面的命令:
...
\paragraph{Cookie Domain}Cookie Domain指定Cookie需要可以访问的范围，在默认情况下，只有和设置cookie的网页在同一个Web服务器的网页才能访问该网页创建的cookie，可以通过Cookie Domain来指定。Cookie Domain主要解决在不同虚拟目录中访问。子域名下可以访问主域名的Cookie，但是主域名不能访问子域名的Cookie。如果访问系统是通过不同的IP来访问，比如应用可以通过内网地址10.10.1.11和外网地址106.13.40.2，如果Cookie Domain设置成内网，那么将导致外网访问时无法获取Cookie，那么设置Cookie Domain就需要留心并保证2个IP都能够获得Cookie。
