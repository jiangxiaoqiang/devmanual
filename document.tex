...
给有线网卡配置地址信息时，不要加默认网关，并查看无线网卡分配到的地址信息中是否有默认网关；使用ip ro 命令查看路由信息，是否有与以下信息类似的内容（这是我机器上的信息，wlp6s0b1是无线网卡并连接外网）：
default via 192.168.1.254 dev wlp6s0b1 proto static metric 1024


注意：两个网卡共存共用的前提是为两个网卡配置不同的网段，使内核知道去往某个网络该走哪个网卡出口。
