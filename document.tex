...
使用一个变量的值时，有助于防止由于空格导致单词分割开和由于识别和扩展了通配符而导致的不必要匹配。除了非常小的脚本（只有几行代码），总是记得用函数来使代码模块化且使得脚本更可读和可重用。在shell中 == 是 = 的同义词，因此仅用个单 = 来做字符串比较。命令代换 是用这个命令的输出结果取代命令本身。用 \$(command) 而不是引号 command 来做命令代换。

\begin{lstlisting}[language=Bash]
user=$(echo “$UID”)
\end{lstlisting}

用readonly来声明静态变量，静态变量不会改变；它的值一旦在脚本中定义后不能被修改。常量和环境变量(Constants and Environment Variable Names)用大写字母命名，而自定义变量用小写。所有的bash环境变量用大写字母去命名，因此用小写字母来命名你的自定义变量以避免变量名冲突。

\subsection{shell调试}

shell跟踪简单的来说就是跟踪shell脚本中的命令的执行。要打开shell跟踪，请使用-x(xtrace或execution trace简称)调试选项。这会让shell在终端上显示所有执行的命令及其参数，如下命令调试运行脚本onekey-deploy.sh。

\begin{lstlisting}[language=Bash]
bash -x onekey-deploy.sh
\end{lstlisting}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.4]{shellscriptdebug.png}
	\caption{Shell脚本调试}
	\label{fig:shellscriptdebug}
\end{figure}

-n(noexec或no ecxecution简称)激活语法检查模式。它会让 shell 读取所有的命令，但是不会执行它们，它（shell）只会检查语法。如果脚本没有语法错误，就没有任何输出，如果有语法错误，则会进行提示：

\begin{lstlisting}[language=Bash]
bash -n onekey-deploy.sh
\end{lstlisting}
...
sudo mkdir -p /opt/dolphin/{frontend/{dist},backend/{log,app},spider}
\end{lstlisting}
...
size默认单位是b，而它代表的是512字节，所以2表示1K，1M则是2048，如果不想自己转换，可以使用其他单位，如c、K、M等。在指定目录下查找指定范围大小的文件：

\begin{lstlisting}[language=Bash]
# 在根目录下查找xls/xlsx/mp3文件且文件大小大于1MB
sudo find / -regex '.*\.xls\|.*\.xlsx\|.*\.mp3' -size +1M
\end{lstlisting}
...
c创建新的档案文件。如果用户想备份一个目录或是一些文件，就要选择这个选项。相当于打包。-z ：是否同时具有 gzip 的属性？亦即是否需要用 gzip 压缩或解压？ 一般格式为xx.tar.gz或xx. tgz。-f ：使用档名，请留意，在 f 之后要立即接档名喔！不要再加其他参数！
...
set命令显示(设置)shell变量 包括的私有变量以及用户变量，不同类的shell有不同的私有变量 bash,ksh,csh每中shell私有变量都不一样。env命令显示(设置)用户变量变量。export命令显示(设置)当前导出成用户变量的shell变量。在.bashrc脚本中，删除了export定义的环境变量并重新source脚本后，在env命令中还是可以查看到定义的变量。不知道是什么原因。
...
占用资源较GNOME ，KDE少。适合老机器，轻量级桌面。与windows界面环境类似。许多不习惯GNOME 3 ，Unity新桌面的同学，很多选择了XFCE 4.8，包括Linus大神同学。
...
一般提示权限错误是由于用户名密码输入错误。aaa为Windows机器的用户名，uid为当前Fedora账户的名称，gid为当前Fedora用户组的名称。10.55.10.2为Windows共享电脑的IP，data为共享文件夹的名称，与盘符无关。/home/dolphin/software本地Fedora机器的目录。cifs为通用网络文件系统，The Common Internet File System (CIFS) is the standard way that computer users share files across corporate intranets and the Internet. An enhanced version of the Microsoft open, cross-platform Server Message Block (SMB) protocol, CIFS is a native file-sharing protocol in Windows 2000.在Ubuntu下，首先安装cifs工具：

\begin{lstlisting}[language=Bash]
#安装cifs
sudo apt-get install cifs-utils -y
#挂载共享文件夹
sudo mount -t cifs -o username="aaa",uid="hldev",gid="hldev" //10.55.10.2/data /home/hldev/software
#检查挂载是否成功，没有输出法代表挂载成功
sudo mount -a
\end{lstlisting}
...
磁盘被手动挂载之后都必须把挂载信息写入/etc/fstab(File System Table)这个文件中，否则下次开机启动时仍然需要重新挂载。系统开机时会主动读取/etc/fstab这个文件中的内容，根据文件里面的配置挂载磁盘。这样我们只需要将磁盘的挂载信息写入这个文件中我们就不需要每次开机启动之后手动进行挂载了。
...
-Xms表示Max Execute Stack，set initial Java heap size，其中X可以理解成Extended或者Execute。属于扩展选项，Java的参数分为标准选项(java -help命令查看)、X选项(java -X命令查看)、XX选项。Ｘ选项基本就固定了，不再变化。X选项有可能变动，XX选项属于实验性选项。如下命令可以打印所有XX选项的默认值：

\begin{lstlisting}[language=Bash]
java -XX:+PrintFlagsInitial
\end{lstlisting}

JDWP是Java Debug Wire Protocol的缩写，它定义了调试器（debugger）和被调试的Java虚拟机（target vm）之间的通信协议。
...
\subsection{Intellij Idea断点}

有时在调试时想让程序在发生异常的地方停下，可以在Intellij Idea中设置异常断点(Ctrl + Shift + B)。如图所示。

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.6]{javabreakpointsetting.png}
	\caption{Java断点中断设置}
	\label{fig:javabreakpointsetting}
\end{figure}
...
\begin{tabular}{cp{10cm}c}
	...
	& \multicolumn{1}{c}{描述}  \\
	\hline			
	Alt + Left/Right  & 编辑器左右文件切换 \\	
	\hline
\end{tabular}

Ubuntu环境下，使用Intellij Idea进行程序开发，在Idea中有个热键“Ctrl+Alt+Left”用于代码浏览的回退，但是我发现我按下以上热键后总是没有一点反应，很明显，该热键已经被其他程序注册了,发现是Gnome下的"switch-to-workspace-left"命令。用命令打开dconf-editor,取消热键配置，如图\ref{fig:ubuntugnomekeybindings}所示。

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.5]{ubuntugnomekeybindings.png}
	\caption{配置Ubuntu Key Bindings}
	\label{fig:ubuntugnomekeybindings}
\end{figure}
...
在使用Intellij Idea的过程中，有时总是不出现智能提示，不出现智能提示大致分为3种情况。其一是勾选了省电模式，在省电模式下没有智能提示。第二是缓存问题，解决办法删除缓存 file -> invalidate caches，根据提示重启Intellij Idea。 情况三配色问题。解决方法是，切换到其他配色，比如内置配色darchla。自己的情况是属于勾选了省电模式。
...
以上是在开发机器上查看，显示已经编译了http\_realip\_module模块。但是项目中使用的tengine，经过查看没有编译http\_realip\_module模块，所以需要手动将此模块编译进去，从tengine官网上下载好源码，输入如下命令进行编译：
...
不指定prefix，则可执行文件默认放在/usr/local/bin，库文件默认放在/usr/local/lib，配置文件默认放在/usr/local/etc。其它的资源文件放在/usr/local/share。不指定prefix时，如果要卸载这个程序，要么在原来的make目录下用一次make uninstall（前提是make文件指定过uninstall）,要么去上述目录里面把相关的文件一个个手工删掉，比较麻烦。如果指定了prefix，直接删掉一个文件夹就够了。在编译时，还需要指定pcre的版本，因为Ubuntu 16.04 LTS里安装有pcre 3，而部署的电脑上不一定安装有pcre 3，而且pcre 3的源码不容易找到。所以编译命令如下：
...
\begin{lstlisting}[language=Bash]
#添加名为company-internal的远程仓库
git remote add company-internal http://xiaoqiang.jiang@gitlab.com/backend/credit-system.git
\end{lstlisting}
