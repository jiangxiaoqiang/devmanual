...
指定prefix，直接删掉一个文件夹就够了。编译完毕之后，输入如下命令：
...
即可看到http\_realip\_module已经编译到nginx中了。Mac下的openssl路径是/Usr/local/Cellar/openssl/1.0.2k。注意在指定openssl时，应该指定的openssl的源码，而不是openssl的安装路径。

\begin{lstlisting}[language=bash]
./configure --prefix=/Users/dolphin/Downloads/tengine-2.1.2-bin --with-openssl=/Users/dolphin/source/openssl --with-cc-opt="-Wno-deprecated-declarations"
\end{lstlisting}
