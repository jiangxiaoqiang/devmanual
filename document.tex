...
\begin{lstlisting}[language=Bash]
./bin/jstatd -J-Djava.security.policy=jstatd.all.policy -J-Djava.rmi.server.hostname=59.214.215.6 -J-Djava.rmi.server.logCalls=true -p 1011
\end{lstlisting}
...
在Mac OS X升级后，每次连接主机都需要输入密码短语(Passphrase)。而且Tab键智能提示的功能也没有了，真是太不方便了。在配置文件~/.ssh/config中做如下设置即可：
...
可以使用 SSH agent 来帮助我们记住密码短语，无需我们记住每一个密钥对的密码短语，减轻了我们的负担。Keychain 是一个用来方便管理 SSH 密钥对的程序，它能尽最大努力去减少对用户的打扰。实际上，它就是一个 shell 脚本，驱动 ssh-agent 或者 gpg-add 来工作。一个值得注意的特性是，keychain 在多个会话中重复使用同一个 ssh-agent 进程。这意味着您只需要在机器启动时输入一次密码短语即可。
