...
telnet因为采用明文传送报文，安全性不好，很多Linux服务器都不开放telnet服务，而改用更安全的ssh方式。telnet命令用于确定远程服务的状态，这里是确定远程服务器的某个端口是否能访问。判断ssh代理是否已经生效，也可以通过nmap扫描代理服务器，如图\ref{fig:scanproxyserver}所示：

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=0.5]{scanproxyserver.png}
	\caption{Nmap扫描代理服务器}
	\label{fig:scanproxyserver}
\end{figure}
