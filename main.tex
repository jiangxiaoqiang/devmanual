...
\begin{lstlisting}[language=Bash]
curl 'http://localhost:28080/api/article/create' -H "APPID:hlb1451j6d136334gh2" -H "ECHOSTR:sdsaasf" -H "TOKEN:e02016ed4adfda9d9743ab70cf389e9 16 (tel:2016121916):58:02" -H 'Accept-Encoding: gzip, deflate' -H 'Cookie:'  --data '{"title":"a123","content":"123456","category":12,"parentCategory":1}' --compressed
\end{lstlisting}
...
过期判断，容易遭受重放攻击(Replay Attack\footnote{\url{https://en.wikipedia.org/wiki/Replay_attack}})。而且从流量中是可以轻松拿到请求头，对于专业选手来说，相当于大门敞开。采用如下shell脚本生成SHA1串。
...
脚本的目的是要使用不同的时间，随机生成的字符串，保证每次请求的Token都需要不一样，相当于一次一密。传闻二战时德国通讯由于偷懒，传输时使用了重复的“密钥”，导致盟军找到加密规律，破解了通讯的内容。使用如下脚本生成随机token：

\begin{lstlisting}[language=Bash]
#!/bin/sh

readonly APP_ID="jiang"
readonly APP_KEY="xiaoqiang"
ECHOSTR=`head -n 1 /dev/urandom|sed 's/[^a-z0-9]//g'`
CURRENT_TIME=`date '+%Y-%m-%d %H:%M:%S'`
TOKEN=`echo -n ${APP_ID}${APP_KEY}${CURRENT_TIME}${ECHOSTR}|shasum -a 1|awk '{print $1}'`
COMMAND=`curl -H "APPID:hlb11529c136998cb6" -H "TIMESTAMP:$CURRENT_TIME" -H "ECHOSTR:$ECHOSTR" -H "TOKEN:$TOKEN" http://localhost:28080/api/blacklist/page?xdr= | jq '.'`	
\end{lstlisting}
...
\subsection{接口返回(Interface Response)}
...
\subsection{接口版本管理(Interface Version Control)}
