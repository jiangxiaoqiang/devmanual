...
\begin{table}
	...
	\begin{center}
		\begin{tabular}{cp{10cm}c}
			...
			/etc/motd & 登陆后的提示信息\\
			/etc/profile.d/ & 开机登录后运行的程序\\
			/etc/redhat-release & 查看版本文件\\
		\end{tabular}	
	\end{center}
\end{table}

配置文件存放的目录，本意为“etcetera”(附加物)的意思。

\section{Shell}

Linux中, 命令主要包含4种: 可执行程序(executable program),shell的内置命令(builtin), shell函数(function), alias命令(别名).一般情况下，type命令被用于判断另外一个命令是否是内置命令，内建命令和外部命令的区别在于前者不需要使用子进程来执行。它们已经和shell编译成了一体，作为shell工具的组成部分存在。不需要借助外部程序文件来运行。外部命令，是存在于bash shell之外的程序，不是shell程序的一部分。通常位于/bin, /usr/bin, /sbin, /usr/sbin中。

\subsection{Shell四种模式}

interactive login shell：这种模式下，shell首先加载/etc/profile，然后再尝试依次去加载下列三个配置文件之一，一旦找到其中一个便不再接着寻找：
