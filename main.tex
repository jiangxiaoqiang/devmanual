...
过期判断，容易遭受重放攻击(Replay Attack)。而且从流量中是可以轻松拿到请求头，对于专业选手来说，相当于大门敞开。采用如下shell脚本生成认证请求的Key。
...
脚本的目的是要使用不同的时间，随机生成的字符串，保证每次请求的Token都需要不一样，相当于一次一密。传闻二战时德国通讯由于偷懒，传输时使用了重复的“密钥”，导致盟军找到加密规律，破解了通讯的内容。
