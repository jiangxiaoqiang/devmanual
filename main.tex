...
项目的一个特点是，有两个彼此独立的网络，所以在调试部署程序时，使用了代理机器。由于代理机器不是服务器7*24小时运行，所以有可能某一个代理机器处于Shutdown状态，此时部署时在脚本中探测每个代理机器是否Up，从而Pick相应活动的代理机。可以保证只要启动任意一个代理机，即可调试部署程序，后面发现暂时没有写脚本的必要，所以未实现。
